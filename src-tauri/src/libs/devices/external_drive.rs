...
// Define UserMetadata according to your actual data structure
...
impl Device for ExternalDrive {
    fn list_books(&self) -> Vec<DeviceBook> {
        ...
        let p: Vec<Item> = serde_json::from_str(&cache).unwrap();
        
        let books = p.iter().map(|item| {
            DeviceBook {
