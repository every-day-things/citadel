...
export const commands = {
	async calibreLoadBooksFromDb(libraryRoot: string): Promise<LibraryBook[]> {
		return await TAURI_INVOKE(
			"plugin:tauri-specta|calibre_load_books_from_db",
			{ libraryRoot },
		);
	},
	async calibreListAllAuthors(libraryRoot: string): Promise<LibraryAuthor[]> {
		return await TAURI_INVOKE("plugin:tauri-specta|calibre_list_all_authors", {
			libraryRoot,
		});
	},
	async calibreSendToDevice(
		libraryRoot: string,
		deviceMountDir: string,
		book: LibraryBook,
	): Promise<null> {
		return await TAURI_INVOKE("plugin:tauri-specta|calibre_send_to_device", {
			libraryRoot,
			deviceMountDir,
			book,
		});
	...
	async initClient(libraryPath: string): Promise<CalibreClientConfig> {
		return await TAURI_INVOKE("plugin:tauri-specta|init_client", {
			libraryPath,
		});
	},
	async getImportableFileMetadata(file: ImportableFile): Promise<{
		file_type: ImportableBookType;
		title: string;
		...
		return await TAURI_INVOKE(
			"plugin:tauri-specta|get_importable_file_metadata",
			{ file },
		);
	},
	async checkFileImportable(
		pathToFile: string,
	): Promise<{ path: string } | null> {
		return await TAURI_INVOKE("plugin:tauri-specta|check_file_importable", {
			pathToFile,
		});
	},
	async addBookToDbByMetadata(
		libraryPath: string,
		md: ImportableBookMetadata,
	): Promise<null> {
		return await TAURI_INVOKE(
			"plugin:tauri-specta|add_book_to_db_by_metadata",
			{ libraryPath, md },
		);
	},
	async updateBook(
		...
		bookId: string,
		updates: BookUpdate,
	): Promise<__Result__<number, null>> {
		try {
			return {
				status: "ok",
				data: await TAURI_INVOKE("plugin:tauri-specta|update_book", {
					libraryPath,
					bookId,
					updates,
				}),
	...
	async isValidLibrary(libraryRoot: string): Promise<boolean> {
		return await TAURI_INVOKE("plugin:tauri-specta|is_valid_library", {
			libraryRoot,
		});
	},
	async createLibrary(libraryRoot: string): Promise<__Result__<null, string>> {
		try {
			return {
				status: "ok",
				data: await TAURI_INVOKE("plugin:tauri-specta|create_library", {
					libraryRoot,
				}),
			};
		} catch (e) {
...
export type LibraryBook = {
	...
	uuid: string | null;
	title: string;
	author_list: LibraryAuthor[];
	sortable_title: string | null;
	author_sort_lookup: { [key in string]: string } | null;
	file_list: BookFile[];
